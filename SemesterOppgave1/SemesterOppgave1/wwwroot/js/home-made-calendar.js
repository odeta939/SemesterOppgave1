...
function createCalendar(direction, date) { //Creating the calendar one month at the time
    $(".calendar" + "." + direction).append( //Setting titel of the month
        $('<div/>')
            .addClass("calendar-titel")
            .append("<span/>")
            .text(direction)
    );
    ...
    if (direction == "outbound") { //Now we are getting the routes from the database and plotting it into the calendar again based on direction
...
function startNewMonth(monthName, date, direction) {//All months are based on direction so it easy to track
    $(".calendar" + "." + direction).append( //Make a new month
        $('<div/>')
            .addClass("month")
    );
    $(".calendar" + "." + direction).children().last().append( //Add month text + arrows
        $('<div/>')
            .addClass("monthText")
            .addClass(direction)
    );
    ...
    for (let days = 0; days < daysOfTheWeeks.length; days++) { //Add the days of the week to make it look more readible
        $(".calendar" + "." + direction).children().last().append(
            $('<div/>')
    ...
    for (let day = 1; day < startingWeekDay; day++) { //We create empty days so that the calendar starts on the right day
        $(".calendar" + "." + direction).children().last().append(
            $('<div/>')
                .addClass("emptyDay")
                .append("<span/>")
    ...
    for (let days = 0; days < 31; days++) { //For every remaining day we create days in the calendar
        let today = new Date();
        let newDay = new Date();
        newDay.setDate(date.getDate() + days) //New day in the calendar will be added days at while going through the for loop

        if (direction == "outbound") { //The difference in months will be calculated here based on direction
        ...
        if (yesterday.getDate() > newDay.getDate() && days > 0) { //When newDay is in a new month we stop and set the days to 100
            days = 100;
        } else {
            if (today.getDate() >= newDay.getDate() && today.getMonth() == newDay.getMonth()) { //If day is before today we set it to notActive
                $(".calendar" + "." + direction).children().last().append( 
                    $('<div/>')
            ...
            } else { //Otherwise we have a normal day
                $(".calendar" + "." + direction).children().last().append(
                    $('<div/>')
                        .addClass("day") //day class so that they all look the same
                        .addClass(direction) //direction so that we can track the days
...
function onClickListnersMonths(direction) {
    ...
    if (today.getMonth() == visibleMonth) { //If the showing month is the same as the visible month
    ...
    if (backPossible) { //if back is possible we add the needed onclick
        $(".arrow.left" + "." + direction).each(function () {
            if ($(this).hasClass("outbound")) { //We check for direction in order to prevent the function from running twice
                $(this).click(function () {
                    arrowOnClick(-1, direction) //We want to go back so we reduce the amount of months with 1
                });
            } else if ($(this).hasClass("inbound")) { //We check for direction in order to prevent the function from running twice
                $(this).click(function () {
                    arrowOnClick(-1, direction) //We want to go back so we reduce the amount of months with 1
                });
            }
...
function arrowOnClick(value, direction) { //We want to change the month of the clicked element
